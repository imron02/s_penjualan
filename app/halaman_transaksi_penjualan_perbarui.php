...
if($_POST)
{
	...
	header("location:?halaman=laporan&jenis_laporan=penjualan");
	exit();
}
...
<form action="" method="post" class="formulir-umum" enctype="multipart/form-data">
	<fieldset>
		...
		<select name="id_pelanggan" >
			<?php 
			$Q = mysql_query("SELECT pelanggan.*,IF(pelanggan.id_pelanggan={$data->id_pelanggan},'selected','') AS pelanggan_selected FROM pelanggan");
			while ($c = mysql_fetch_object($Q)):
				echo '<option value="'.$c->id_pelanggan.'" '.$c->pelanggan_selected.'>'.$c->nama_pelanggan.'</option>';
			endwhile;
			?>
		</select>
		<hr />
